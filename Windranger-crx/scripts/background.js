...
var WindrangerBack = {
  ...
  requestFunc: {
    ...
    getData: function (req, sendRes) {
      ...
      // 获取siteList
      var siteList = localStorage.getItem(WindrangerBack.config.cacheKey.siteList);
      if (!siteList) {
        returnData = {
          'status': 1,
          'msg': 'siteList error'
        ...
        // 获取server
        var server = localStorage.getItem(WindrangerBack.config.cacheKey.server);
        if (!server) {
          returnData = {
            'status': 1,
            'msg': 'server error'
    ...
    saveServer: function (req, sendRes) {
      ...
      WindrangerBack.createServer(host, port);

      // 重新设置代理
      WindrangerBack.getPacScript(true);
      WindrangerBack.checkSwitch();
    ...
    saveSite: function (req, sendRes) {
      ...
      var savedItem = false;
      var siteListCache = WindrangerBack.getSiteList();
      ...
      if (savedItem) {
        ...
        localStorage.setItem(WindrangerBack.config.cacheKey.siteList, JSON.stringify(siteList));
        // 重新设置代理
        WindrangerBack.getPacScript(true);
        WindrangerBack.checkSwitch();
      }
  ...
  proxy: {
    on: function () {
      // 检查缓存中是否有pac script
      var pacScript = localStorage.getItem(WindrangerBack.config.cacheKey.pacScript);
      if (!pacScript) {
        // 没有，使用默认server和site list生成
        pacScript = WindrangerBack.getPacScript();
      }
